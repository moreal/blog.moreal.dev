...
의미한다고 봐야하는가? 에 대해 찾아본 것을 기록하고 싶어서 적는 단락이다.
LISP은 방언이 매우 많다고 들었는데 그 중 최근에 들어서 아는 것으로는
[Common Lisp]과 [Clojure]가 있다. 근데 “LISP의 방언이 매우 많다” 라는 것 자체가
LISP이라는 것이 있다는 건데.. [위키피디아 문서][ko-wikipedia-lisp]를 보니 존
매카시라는 분이 MIT에서 1960년에 발표한 논문이 기반이고 이후 여러 실험을 통해
기능들이 생겨나고 집합이 되어 [Common Lisp]이나 [Scheme] 처럼 별개의
방언(언어)으로 취급되는 것 같다.
